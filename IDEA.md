...
- **Fallback automático configurado e depois REMOVIDO (05-06/08/2026)**: em 05/08 foi definido `fallback_providers` = `{provider: nous, model: google/gemini-3.5-flash-20260519}` no config.yaml global. Em 06/08, a pedido do usuário, foi **removido** (`hermes config unset fallback_providers`) — a preferência agora é: se o DeepSeek falhar com erro transitório (503, rate-limit, timeout), **tentar novamente com o próprio DeepSeek** (até 3 tentativas, conforme regra adicionada ao prompt do cron), nunca trocar para outro modelo. O Hermes já aplica retry automático em 5xx no nível do scheduler. Obs.: o wizard `hermes fallback add` não é scriptável em terminal não-TTY; o formato de dict único funciona porque `_iter_fallback_entries` em `hermes_cli/fallback_config.py` aceita tanto dict quanto lista.
