...
- **05/08/2026**: trocado para DeepSeek V4 Flash 0731 (`deepseek/deepseek-v4-flash-20260731`). O job de 5h falhou com `HTTP 503` (capacidade temporariamente indisponível no provedor — não foi erro de configuração). Re-executado manualmente às 08:09 com sucesso.
  - **Resultado da revisão**: o melhor resultado dos 4 dias testados, superando inclusive o Sonnet 5 do dia 02/08. 5-7 itens ricos por categoria, cada um com data específica, fontes verificadas (conferi manualmente 2 ao acaso — URL exata do Hacker News e detalhes do Qwen3.8-Max batem 100% com a realidade). Seção `comparativos_modelos` excepcional: 5 comparativos detalhados com Elo, preço por tarefa e recomendação por caso de uso. Único problema encontrado: nenhum de fonte, apenas a falha pontual de disponibilidade do provedor às 5h.
  - **Conclusão do teste A/B**: DeepSeek V4 Flash 0731 é o vencedor — melhor qualidade que Sonnet 5, custo drasticamente menor ($0.01/$0.02 por 1M tokens vs $2-8/$12-40 do Sonnet). Risco identificado: disponibilidade/estabilidade do provedor pode falhar ocasionalmente (HTTP 503) — vale considerar fallback automático para outro modelo caso o job falhe (ver `hermes fallback add`), ou apenas monitorar e rodar manualmente quando acontecer.
- **Modelo definitivo escolhido**: `deepseek/deepseek-v4-flash-20260731` (provider `nous`), mantido no cron job a partir de 05/08/2026.
- **Fallback automático configurado (05/08/2026)**: `fallback_providers` em `~/.hermes/config.yaml` (ou `%LOCALAPPDATA%\hermes\config.yaml` no Windows) definido como `{provider: nous, model: google/gemini-3.5-flash-20260519}`. Se o DeepSeek falhar (rate-limit, 5xx, erro de conexão), o Hermes troca automaticamente para o Gemini 3.5 Flash sem perder a conversa/execução. **Atenção: essa cadeia de fallback é GLOBAL** — vale para a sessão principal, todas as delegações (`delegate_task`) e todos os cron jobs, não só o Radar IA. Configurado via `hermes config set fallback_providers.provider nous` + `hermes config set fallback_providers.model google/gemini-3.5-flash-20260519` (o `hermes fallback add` interativo não é scriptável em terminal não-TTY; o formato de dict único funciona porque `_iter_fallback_entries` em `hermes_cli/fallback_config.py` aceita tanto dict quanto lista).
